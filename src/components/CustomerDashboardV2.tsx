...
const CustomerDashboardV2 = () => {
  ...
  React.useEffect(() => {
    ...
    } else if (['/customer/appointments', '/customer/shop', '/customer/promotions'].some(p => path.startsWith(p))) {
      setSelectedCategory('services');
    } else if (['/customer/orders', '/customer/accounts'].some(p => path.startsWith(p))) {
